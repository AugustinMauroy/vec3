...
import { Vec3, v } from "./mod.ts";

describe("v()", { concurrency: true }, () => {
	it("no args", () => {
		const v1 = v();
		...
		assert.ok(v1 instanceof Vec3);
	});

	it("1 arg (fallback to 0, 0, 0)", () => {
		const v1 = v(5);
	...
	it("deserialize", () => {
		...
		assert.ok(v2 instanceof Vec3);
	});

	it("invalid deserialize", () => {
		assert.throws(() => v("this not a vector"), /cannot parse/);
	});
});

describe("vec3", { concurrency: true }, () => {
	it("isZero", () => {
	...
	it("multiply array", () => {
		...
		const v2 = v1.multiply([new Vec3(-1, -2, -3), new Vec3(1, 2, 3)]);

		assert.strictEqual(v2, v1);
		assert.strictEqual(v1.x, -1);
		assert.strictEqual(v1.y, -8);
		assert.strictEqual(v1.z, -27);
	});
	...
	it("divide array", () => {
		...
		const v2 = v1.divide([new Vec3(2, 5, 3), new Vec3(1, 2, 3)]);

		assert.strictEqual(v2, v1);
		assert.strictEqual(v1.x, 5);
		assert.strictEqual(v1.y, 2);
		assert.strictEqual(v1.z, 3.3333333333333335);
	});
